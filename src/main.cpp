 ...
 *******************************************/
static const double degree25_radian = 0.436332;
static const double latency = 0.1;  //100 milisecond
static const double mph2ms = 0.44704; //convert mph to m/s

int main(int argc, char* argv[]) {

  /***************************
   * accept command argument for tuning
   *
   *******************/
  if (argc == 8) {
    ...
    for (size_t i = 0; i < 7; i++) {
      weights.push_back(std::stod(string(argv[i + 1])));
      cout << weights[i] << " ";
    }
    cout << endl;
    MPC::weights = weights;
        ...
        if (s != "") {
          ...
          if (event == "telemetry") {
            ...
            for(size_t i=0; i< ptsx.size();i++) {
              ...
              double y_car = -(ptsx[i] - px) * sin(psi) + (ptsy[i] - py)* cos(psi);
            ...
            double gradient = coeffs[1] + 2* coeffs[2] * px_car + 3* coeffs[3]*px_car*px_car;
            double epsi = psi_car - atan(gradient);

            Eigen::VectorXd state(6);
            state<< px_car,py_car,psi_car,v,cte,epsi;

            //we need the last actuators values to predict states due to latency.
            Eigen::VectorXd actuators(2);
            actuators << - steering_angle, throttle_value;

            //Solve is extended to accept actuators and latency.
            auto results= mpc.Solve(state,actuators,coeffs,latency);
